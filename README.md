...
- **空间洞察**：选任意目录，只读分析其中的**最大文件 Top 榜**与**重复文件组**，定位空间去向
...
| 📊 **空间洞察** | 任意目录只读分析：最大文件 Top 榜 + 重复文件组（按大小排序，标出可释放量） |
...
2. 工具**只读**分析该目录（不改任何文件）
3. 切换「最大文件 / 重复文件」两种视图：Top 榜按大小倒序；重复组列出每份路径和可释放量
...
- **空间洞察**：选任意目录只读分析，最大文件 Top 榜 + 重复文件组（标出可释放量）
